...
impl AppStateTokenClaims {
	...
	pub async fn encode(uri: String) -> Result<String, String> {
		let app_state_token_claim = Self { uri: Some(uri), exp: jsonwebtoken::get_current_timestamp() + 3600 };

		let creds = azure_identity::create_credential().map_err(|e| format!("Failed to create Azure credentials: {e:?}"))?;
		let client = match KeyvaultClient::new("https://eggappserverkeyvault.vault.azure.net", creds) {
